...
interface TopUpTapToPayModalProps {
  visible: boolean;
  onClose: () => void;
...
const TopUpTapToPayModal: React.FC<TopUpTapToPayModalProps> = ({
  visible,
  onClose,
  tapToPayAddress,
  onSuccess,
  ...
  // Find USDC on Solana specifically (not Ethereum USDC)
  const usdcAsset = assets.find(
    a =>
      a.symbol === 'USDC' &&
      (a.id === 'usd_coin_(solana)' || a.name?.includes('Solana')),
  );

  const solBalance = solAsset?.balance || 0;
  const usdcBalance = usdcAsset?.balance || 0;

  const currentBalance = selectedAsset === 'SOL' ? solBalance : usdcBalance;

  console.log('[TopUpModal] Asset balances:', {
    solBalance,
    usdcBalance,
    selectedAsset,
    currentBalance,
    solAsset: solAsset ? `found: ${solAsset.name}` : 'not found',
    usdcAsset: usdcAsset
      ? `found: ${usdcAsset.name} (${usdcAsset.id})`
      : 'not found',
    allUSDCAssets: assets
  ...
  const handleTopUp = async () => {
    ...
    if (selectedAsset === 'SOL' && transferAmount > solBalance - 0.01) {
      onError('Please reserve at least 0.01 SOL for transaction fees');
      return;
    }

    if (!addresses.solana || !privateKeys.solana) {
    ...
    try {
      ...
      console.log(`[TopUp] Starting ${selectedAsset} transfer:`, {
      ...
      if (selectedAsset === 'SOL') {
        ...
        result = await transferSolana({
          ...
          toAddress: tapToPayAddress,
          amount: transferAmount,
          privateKey: privateKeys.solana,
          asset: {
            id: 'solana',
            symbol: 'SOL',
            name: 'Solana',
        ...
        console.log('[TopUp] Initiating USDC SPL token transfer...');
        ...
        console.log('[TopUp] USDC transfer result:', result);
      }

      if (result.success) {
        console.log(`[TopUp] Transfer successful. TxHash: ${result.txHash}`);
        setAmount('');
        setIsProcessing(false); // Stop processing immediately

        // Close modal and notify parent
        onClose();
        onSuccess(`Successfully topped up ${transferAmount} ${selectedAsset}`);

        return; // Exit early to prevent finally block from running
      } else {
        console.error('[TopUp] Transfer failed:', result.error);
        onError(result.error || 'Transfer failed');
      }
    } catch (error: any) {
      console.error('[TopUp] Exception during transfer:', error);
      console.error('[TopUp] Error details:', {
        message: error.message,
        stack: error.stack,
        name: error.name,
      });
      onError(error.message || 'Failed to top up');
    } finally {
  ...
  const isValidAmount =
    ...
    (selectedAsset === 'SOL' ? parseFloat(amount) <= solBalance - 0.01 : true); // For USDC, only check if amount <= balance

  // Debug logging
  useEffect(() => {
    if (amount) {
      console.log('[TopUpModal] Validation debug:', {
        selectedAsset,
        amount,
        currentBalance,
        solBalance,
        ...
        isValidAmount,
        hasAmount: !!amount,
        isPositive: parseFloat(amount) > 0,
        isWithinBalance: parseFloat(amount) <= currentBalance,
      });
    }
  }, [
    amount,
              ...
              >
                ...
                {selectedAsset === 'SOL' && (
                  ...
                  >
                    ...
                    >
                      Note: 0.01 SOL will be reserved in your main wallet for
                      transaction fees
                    </Text>
                  </View>
                )}
                ...
                >
                  {isProcessing ? (
                    ...
                    >
                      Top Up {amount ? `${amount} ${selectedAsset}` : ''}
                    </Text>
                  )}
                </TouchableOpacity>
              </ScrollView>
...
export default TopUpTapToPayModal;
